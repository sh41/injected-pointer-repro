...
- `testPointersIntoTwoInjectionsOverTheSameHostRangeEachRestoreIntoTheirOwnLanguage` — XML and plain-text injections over
  ...
  `Cannot restore ... from injected{...}; restored=null`.
- `testWhichPointerFailsVariesBetweenIdenticalRuns` — the same scenario 1000 times in one JVM. Exactly one pointer fails
  each time, but which language loses varies: one run of this test gave `TEXT failed 378, XML failed 622`.
